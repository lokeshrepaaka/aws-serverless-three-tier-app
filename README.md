...
[🚀 **Open Live Application**](https://d3ql7kg50manzy.cloudfront.net) · [🖼️ **View Architecture Diagram**](./diagrams/aws-serverless-three-tier-architecture.png) · [⚙️ **CI/CD Pipeline**](#cicd-pipeline)
...
[![AWS Serverless Three-Tier Application Architecture](./diagrams/aws-serverless-three-tier-architecture.png)](./diagrams/aws-serverless-three-tier-architecture.png)

### Request Flow

1. **CloudFront** serves the frontend from a private **S3** bucket.
2. **Amazon Cognito** handles registration, email verification, and authentication.
3. The browser sends the Cognito JWT to **API Gateway**.
4. API Gateway validates the JWT before invoking **Lambda**.
5. Lambda performs task CRUD operations against **DynamoDB**.
6. The Cognito `sub` is stored as `owner_id`, keeping each user's tasks isolated.
